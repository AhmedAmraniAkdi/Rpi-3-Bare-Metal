...
void join_all(void){
	...
	demand(core_tasks[0].tasks_num == 0, "didn't join all tasks from all cores successfully, num of tasks %d\n", core_tasks[0].tasks_num);
}

extern void reset_main_stack();
...
void secondary_cores_threading_init(void){
	...
	//DISABLE_CORE_TIMER();
	disable_core_timer_int();
	demand(core_tasks[core].tasks_num == 0, "not idle core %d went to sleep, num of tasks %d\n", core, core_tasks[core].tasks_num);
}
